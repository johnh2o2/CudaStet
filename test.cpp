...
void timing(int Nmin, int Nmax, int Ntests) {
    ...
    for (int testtype = 0; testtype < 4; testtype ++){
    	printf("timing : (%s, %s)\n", testtype%2 ? "GPU" : "CPU", 
    		testtype/2 ? "CONSTANT" : "EXPONENTIAL");
	    ...
	    for (int i = 0; i < Ntests; i++) {
	    	...
	    	weight_type wtype = testtype/2 ? CONSTANT : EXP;
	    	
	    	start = clock();

	    	// calculate Stetson J
	   		if (testtype % 2)
	   			stetson_j_gpu(x, f, err, wtype, n);
	   		else
	   			stetson_j_cpu(x, f, err, wtype, n);

	    	dt = clock() - start;
	    	
	    	// output
	    	printf("%-10d %-10.3e\n", n, 
	    		           ((real_type) dt / CLOCKS_PER_SEC));

	    	free(x); free(f); free(err);
	    }
	}
}
...
int main(int argc, char *argv[]) {
    if (!((argc == 4 and argv[1][0] == 's') || (argc==5 and argv[1][0] == 't'))) {
    	...
    	fprintf(stderr, "       [timing test]  (2) %s t "
    								"<nmin> <nmax> <ntests>\n", argv[0]);
